...
let state = {
  productSelected: MOCK_PRODUCT_RESULT_LIST[1]
};
...
describe("ProductModal", () => {
  ...
  beforeEach(() => {
    ...
    wrapper = shallowMount(ProductModal, {
      router,
      store,
      localVue,
      propsData: {
        product: MOCK_PRODUCT_RESULT_LIST[0]
      }
    });
  });
  ...
  it("Should be created", () => {
    ...
    expect(wrapper.find(".product-info h1").text()).toEqual(
      MOCK_PRODUCT_RESULT_LIST[1].title
    );
  });

  it('Should commit "setProductSelected" on close modal click', () => {
    const openModalBtn = wrapper.find(".modal-header button");
    openModalBtn.trigger("click");
    expect(mutations.setProductSelected).toHaveBeenCalledWith(state, undefined);
  });

  it('Should dispatches "updateCart" when clicking on add Cart', async () => {
    const button = wrapper.find(
      ".quick-add-to-cart .cart .single_add_to_cart_button"
    );
  ...
  it('Should commit "setProductSelected" on destroy component', () => {
    wrapper.vm.$destroy();
    expect(mutations.setProductSelected).toHaveBeenCalledWith(state, undefined);
  });
});
